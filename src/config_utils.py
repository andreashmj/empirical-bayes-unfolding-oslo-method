"""
Configuration loading for the unfolding run pipeline.

The project uses three YAML configuration layers:

1. run configs:
   Top-level files that point to the data, prior, sampler, and output paths
   for one unfolding run.

2. prior configs:
   Named prior profiles and excitation-energy mapping rules. A profile is
   selected for each requested Ex row before constructing the empirical-Bayes
   prior.

3. sampler configs:
   A complete base sampler configuration, optionally combined with named
   profile overrides. The selected sampler profile is deep-merged into the
   base configuration for each Ex row.

This module does not build the statistical model and does not run inference.
It only loads configuration files, resolves Ex-dependent profiles, and checks
that the resulting sampler configuration has the fields required by the run
pipeline.
"""
...
def dict_to_namespace(obj: Any) -> Any:
    """Recursively convert dictionaries to argparse.Namespace objects."""
...
def _deep_update(base: dict[str, Any], updates: dict[str, Any]) -> dict[str, Any]:
    """Return a recursive update of 'base' without modifying the inputs."""
...
class ProfileMapper:
    """Resolve a named profile for one excitation energy.
    A mapping rule can either list exact excitation energies,
        energies: [2500, 4000]
    or define an excitation-energy range,
        range: [3000, 6000]
    Exact-energy rules take precedence over range rules. Range rules should be
    interpreted consistently with the code, preferably as half-open intervals
    [lower, upper), so adjacent ranges can meet at a boundary without
    ambiguity. Returned profiles are deep copies, so callers can modify the
    result without changing the stored profile definitions.
    """
    ...
    def __init__(self, profiles: dict[str, Any], mapping: list[dict[str, Any]]):
        ...
        for rule in mapping:
            ...
            if has_range == has_energies:
                raise ValueError(
                    "Each profile mapping rule must contain exactly one of "
                    "'range' or 'energies'."
                )
...
class SamplerMapper:
    """Build the sampler configuration for one excitation-energy row.

    The sampler YAML contains a complete base configuration and can optionally
    contain named profile overrides. For a requested Ex value, the matching
    profile is deep-merged into the base configuration and the final result is
    validated.

    Profiles are intended for Ex-dependent tuning choices such as target
    acceptance or tree depth. They must not change the sampler backend, so
    nuts.nuts_sampler is kept fixed for a given benchmark or production run.
    """
    ...
    def config_for(self, ex_energy: float) -> dict[str, Any]:
        """Return the sampler configuration for one excitation-energy row."""
...
def _validate_pymc_backend_config(config: dict[str, Any]) -> None:
    """Validate optional PyMC backend settings."""
...
def _validate_jax_config(config: dict[str, Any]) -> None:
    """Validate optional JAX backend settings."""
...
def _validate_nutpie_config(config: dict[str, Any]) -> None:
    """Validate optional Nutpie backend settings."""
